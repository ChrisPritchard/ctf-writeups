...
echo "git clone https://github.com/NHAS/reverse_ssh && cd reverse_ssh && git checkout unstable && RSSH_HOMESERVER=$(hostname -i):3232 make && cd bin/ && cp ~/.ssh/id_ed25519.pub authorized_keys && ./server --homeserver_address 192.168.122.1:1234 --enable-webserver $(hostname -i):3232 & && python3 -m http.server 1234 &" > reverse_ssh.sh
